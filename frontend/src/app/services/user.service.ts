...
export class UserService {
  ...
  public save(user: any){
    ...
    if (user._id == 'new'){
      return this.http.post(`${environment.api}/users`,saveable);
    } else {
      return this.http.put(`${environment.api}/users/${user._id}`,saveable);
    }
  }

  public delete(_id: string){
    this.http.delete(`${environment.api}/users/${_id}`).subscribe( () => this.get() );
  }
